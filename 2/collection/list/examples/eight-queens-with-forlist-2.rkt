  ...
  (define (queen-cols k)
    (if (= k 0)
        ...
        (for*/list ([rest-of-queens (queen-cols (- k 1))]
                    ...
                    #:when (safe? new-row k rest-of-queens))
          (adjoin-position new-row
                           k
                           rest-of-queens))))
  (queen-cols board-size))
...
(define (singleon empty-board)
 '((1 1)))

(define (safe? new-row k positions)
  (let ((q1r new-row)
        (q1c k))
    (all?
     (lambda (position)
...
(define sicp-2.42-tests
  (test-suite
   ...
   (check-true (safe? 3 2 '((1 1))))
   (check-false (safe? 1 2 '((1 1))))

   (check-true (safe? 1 2 '((3 1))))
   (check-false (safe? 2 2 '((3 1))))
   (check-false (safe? 2 2 '((1 1))))
