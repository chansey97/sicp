...
(define (install-polynomial-package)
  ...
  (define (make-const p n)
    (tag (make-poly (variable p)
                    (adjoin-term (make-term 0 (make-scheme-number n))
                                 (the-empty-termlist)))))
  ...
  (put 'add '(scheme-number polynomial) (lambda (n p) (add (make-const p n) (tag p))))
  (put 'mul '(scheme-number polynomial) (lambda (n p) (mul (make-const p n) (tag p))))
  (put 'add '(polynomial scheme-number) (lambda (p n) (add (make-scheme-number n) (tag p))))
  (put 'mul '(polynomial scheme-number) (lambda (p n) (mul (make-scheme-number n) (tag p))))
