#lang racket
(provide cm-operations cm-registers the-global-environment)
...
(define (primitive-implementation proc)
  ;; (println "primitive-implementation proc")
  ;; (println proc)
  (cadr proc)
  )
...
(define (apply-primitive proc args)
  (apply (car (cdr proc)) (mlist->list args)))

(define primitive-procedures
  (list (list 'car mcar) ;; r5rs or racket ???? mcar?
        (list 'cdr mcdr)
        (list 'cons mcons)
        (list 'null? null?)
        (list 'pair? mpair?)
        ...
        (list 'apply-in-underlying-scheme apply-primitive)
        )
  )
...
(define (apply-primitive-procedure proc args)
  ;; (println "apply-primitive-procedure proc")
  ;; (println proc)
  ;; (println "apply-primitive-procedure args")
  ;; (println args)
...
(define cm-registers '(arg1 arg2 exp env val proc argl continue unev))
