...
;; ((lambda ()
;;    (define (while-loop)
;;      (if condition body 'done))
;;    (while-loop)))

;; "while-loop" have name issue:
;; If client code call a void function named "while-loop", it will error matching.
;; We can forbid "while-loop" in syntax analyzer to workaround it. 
;; Another solution is use y-combinator, see ../basic-extend-syntax-while-with-y
